...
export type Variant = "compact" | "minimal";
type Limit = {
  label: string;
...
export function mountUsage(
  ...
  shadow.host.setAttribute(
    "aria-label",
    `Codex subscription usage for ${context.instanceId}`,
  );
...
function render(
  ...
  if (snapshot.status === "unavailable") {
    article.innerHTML = `<div class="state">${snapshot.reason === "sign_in" ? "Sign in with Codex" : "Usage unavailable"}</div>${variant === "minimal" ? "" : refreshButton(refreshing)}`;
  ...
  article.innerHTML = `<header><h2>Codex Usage</h2><span class="meta">${titleCase(snapshot.subscription_type)}</span>${refreshButton(refreshing)}<span class="updated">Updated ${relative(snapshot.updated_at, true)}${snapshot.stale ? " - Stale" : ""}</span></header><div class="limits">${limitMarkup(important, true, true, displayMode)}</div>`;
  bindRefresh(article, onRefresh);
}
