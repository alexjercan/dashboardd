...
function render(
  ...
  article.innerHTML = `<header><h2>Codex Usage</h2><span class="meta">${titleCase(snapshot.subscription_type)}</span>${refreshButton(refreshing)}<span class="updated">Updated ${relative(snapshot.updated_at, true)}${snapshot.stale ? " - Stale" : ""}</span></header><div class="limits">${limitMarkup(important, true, true)}</div>`;
  bindRefresh(article, onRefresh);
}
