...
try {
  ...
  const dashboardStorageBeforeSurface = await page.evaluate(() => ({
    dashboards: localStorage.getItem("dashboardd.dashboards/v1"),
    bindings: localStorage.getItem("dashboardd.instance-bindings/v1"),
  }));
  const standaloneCpuResponse = await page.request.post(
    `${dashboardApiUrl}/instances`,
    { data: { widget_id: "cpu", variant_id: "full" } },
  );
  assert.equal(standaloneCpuResponse.status(), 201);
  const standaloneCpu = await standaloneCpuResponse.json();
  const cpuSurface = await context.newPage();
  pages.push(cpuSurface);
  await cpuSurface.goto(
    `${baseUrl}/surface/${encodeURIComponent(standaloneCpu.id)}`,
  );
  assert.equal(
    await cpuSurface.locator("#widget").getAttribute("data-presentation"),
    "focus",
  );
  await waitForTelemetry(cpuSurface.locator("#widget"));
  assert.equal(await cpuSurface.locator("#health").isHidden(), true);
  assert.deepEqual(
    await cpuSurface.evaluate(() => ({
      dashboards: localStorage.getItem("dashboardd.dashboards/v1"),
      bindings: localStorage.getItem("dashboardd.instance-bindings/v1"),
    })),
    dashboardStorageBeforeSurface,
    "standalone surfaces do not mutate Dashboard composition",
  );
  const invalidPresentation = await context.newPage();
  pages.push(invalidPresentation);
  await invalidPresentation.goto(
    `${baseUrl}/surface/${encodeURIComponent(standaloneCpu.id)}?presentation=wide`,
  );
  await invalidPresentation
    .locator("#unavailable-message", { hasText: "Invalid widget presentation" })
    .waitFor();
  await cpuSurface.close();
  assert.equal(
    (
      await page.request.get(
        `${dashboardApiUrl}/instances/${encodeURIComponent(standaloneCpu.id)}`,
      )
    ).status(),
    200,
    "closing a standalone surface does not delete its runtime instance",
  );
  const cpuDeletionSurface = await context.newPage();
  pages.push(cpuDeletionSurface);
  await cpuDeletionSurface.goto(
    `${baseUrl}/surface/${encodeURIComponent(standaloneCpu.id)}`,
  );
  await waitForTelemetry(cpuDeletionSurface.locator("#widget"));
  await page.request.delete(
    `${dashboardApiUrl}/instances/${encodeURIComponent(standaloneCpu.id)}`,
  );
  await cpuDeletionSurface
    .locator("#unavailable-message", { hasText: "deleted" })
    .waitFor();
  await cpuDeletionSurface.close();
  await invalidPresentation.close();

  const standaloneFixtureResponse = await page.request.post(
    `${dashboardApiUrl}/instances`,
    {
      data: {
  ...
  assert.equal(standaloneFixtureResponse.status(), 201);
  const standaloneFixture = await standaloneFixtureResponse.json();
  const fixtureSurface = await context.newPage();
  pages.push(fixtureSurface);
  await fixtureSurface.goto(
    `${baseUrl}/surface/${encodeURIComponent(standaloneFixture.id)}?presentation=tile`,
  );
  assert.equal(
    await fixtureSurface.locator("#widget").getAttribute("data-presentation"),
    "tile",
  );
  await fixtureSurface
    .locator('.fixture-input:text-is(\'{"text":"surface"}\')')
    .waitFor();
  const updateSurfaceInput = await page.request.put(
    `${dashboardApiUrl}/instances/${encodeURIComponent(standaloneFixture.id)}/inputs`,
    {
      data: {
        inputs: {
          message: {
            type: "dashboardd.fixture-message/v1",
            value: { text: "updated surface" },
          },
        },
      },
    },
  );
  assert.equal(updateSurfaceInput.status(), 200);
  await fixtureSurface
    .locator('.fixture-input:text-is(\'{"text":"updated surface"}\')')
    .waitFor();
  await page.request.post(
    `${dashboardApiUrl}/instances/${encodeURIComponent(standaloneFixture.id)}/messages`,
    { data: { command: "degrade" } },
  );
  await fixtureSurface.locator("#health:not([hidden])").waitFor();
  assert.match(
    await fixtureSurface.locator("#health-message").textContent(),
    /Fixture requested degraded health/,
  );
  await fixtureSurface.locator("#restart").click();
  await fixtureSurface.locator("#health").waitFor({ state: "hidden" });
  const restartedFixtureHealth = await page.request.get(
    `${dashboardApiUrl}/instances/${encodeURIComponent(standaloneFixture.id)}/health`,
  );
  assert.equal((await restartedFixtureHealth.json()).restart_count, 1);
  await page.request.delete(
    `${dashboardApiUrl}/instances/${encodeURIComponent(standaloneFixture.id)}`,
  );
  await fixtureSurface.locator("#unavailable").waitFor();
  await fixtureSurface.close();

  const invalidSurface = await context.newPage();
  pages.push(invalidSurface);
  await invalidSurface.goto(`${baseUrl}/surface/not-an-instance`);
  await invalidSurface
    .locator("#unavailable-message", { hasText: "Invalid runtime instance ID" })
    .waitFor();
